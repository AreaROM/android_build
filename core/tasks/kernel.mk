...
define mv-modules
    ...
    if [ "$$mdpath" != "" ];then\
        ...
        for i in $$ko; do $(ANDROID_KERNEL_TOOLCHAIN_PATH)/arm-eabi-strip --strip-unneeded $$i;\
...
ifeq ($(TARGET_ARCH),arm)
    ...
    ifneq ($(TARGET_KERNEL_CUSTOM_TOOLCHAIN),)
        ifeq ($(HOST_OS),darwin)
            ARM_CROSS_COMPILE:=CROSS_COMPILE="$(ccache) $(ANDROID_BUILD_TOP)/prebuilt/darwin-x86/toolchain/$(ANDROID_KERNEL_TOOLCHAIN_PATH)/bin/arm-eabi-"
        else
            ARM_CROSS_COMPILE:=CROSS_COMPILE="$(ccache) $(ANDROID_BUILD_TOP)/prebuilt/linux-x86/toolchain/$(ANDROID_KERNEL_TOOLCHAIN_PATH)/bin/arm-eabi-"
        endif
    else
        ARM_CROSS_COMPILE:=CROSS_COMPILE="$(ccache) $(ANDROID_KERNEL_TOOLCHAIN_PATH)/arm-eabi-"
    endif
    ccache = 
endif
